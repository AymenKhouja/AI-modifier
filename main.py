...
segmentation_models = [pipeline("image-segmentation", seg_config[i]) for i in range(len(seg_config))]
...
prompts = ["RAW photo, high-quality, completely naked, nude woman"]
...
def process_image(i, file_name, prompt):
    try:
        ...
        inference_pipeline = OutpaintInpaintInfer(segmentation_model=segmentation_models, not_included=not_included, pipe=pipe)
        ...
        result_path = os.path.join(output_dir, f"14th_{i}_{len(prompt)}_image_results.jpg")
